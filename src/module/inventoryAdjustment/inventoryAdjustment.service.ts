...
export class InventoryAdjustmentService extends BaseService<InventoryAdjustment> {
  ...
  protected searchableFields: (keyof InventoryAdjustment)[] = [
    "code",
    "reason",
  ];
  constructor(
    ...
    @inject(INVENTORY_ADJUSTMENT_TYPES.LineRepository)
    private lineRepository: InventoryAdjustmentLineRepository,
  ...
  private async prepareLines(
    ...
    for (const line of lines) {
      if (!line.productId)
        throw new Error("inventory.adjustment.line.product.required");
  ...
  private async prepareInventoryValues(
    ...
    for (const line of lines) {
      ...
      if (!productId || productIds.has(productId))
        throw new Error("inventory.adjustment.line.duplicate_product");
      productIds.add(productId);
      const countedQuantity = Number(
        line.countedQuantity ?? line.quantity ?? 0,
      );
  ...
  async actionAfterDelete(
    ...
    const productIds = new Set<string>(
      ((data as any).lines || [])
        .map((line: InventoryAdjustmentLine) => line.productId)
