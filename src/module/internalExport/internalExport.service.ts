...
import {
  InternalExport,
  InternalExportType,
} from "@/database/models/store/InternalExport";
...
export class InternalExportService extends BaseService<InternalExport> {
  ...
  constructor(
    @inject(INTERNAL_EXPORT_TYPES.Repository)
    repository: InternalExportRepository,
    @inject(INTERNAL_EXPORT_TYPES.LineRepository)
    private lineRepository: InternalExportLineRepository,
  ...
  private async prepareLines(
    ...
    for (const line of lines) {
      if (!line.productId)
        throw new Error("internal.export.line.product.required");
      const quantity = Number(line.quantity) || 0;
      if (!Number.isFinite(quantity) || quantity <= 0)
        throw new Error("internal.export.line.quantity.invalid");
  ...
  private async syncLines(
    ...
    const existing = await repository.find({
      where: { internalExportId: id } as any,
    });
  ...
  private async replay(
    ...
    const replayFrom =
      fromDate && fromDate < item.occurredAt ? fromDate : item.occurredAt;
    for (const productId of productIds) {
      for (const storeId of [
        ...new Set(
          [oldStoreId, item.storeId].filter((value): value is string =>
            Boolean(value),
          ),
        ),
      ]) {
  ...
  async validateBeforeCreate(
    ...
    if (
      !Object.values(InternalExportType).includes(
        data.type as InternalExportType,
      )
    )
  ...
  async update(
    ...
    const run = async (em: EntityManager): Promise<InternalExport | null> => {
      ...
      if (!req?.storeContext?.storeId)
        throw new BadRequestError("Vui lòng chọn cửa hàng đang thao tác");
      if (current.storeId !== req.storeContext.storeId)
        throw new BadRequestError(
          "Không thể cập nhật dữ liệu của cửa hàng khác",
        );
      if (payload.storeId && payload.storeId !== current.storeId)
        throw new BadRequestError(
      ...
      if (
        ...
        !Object.values(InternalExportType).includes(
          payload.type as InternalExportType,
        )
      )
        throw new Error("internal.export.type.invalid");
      ...
      return (
        (await this.repository
          .getRepository(em)
          .findOne({ where: { id }, relations: { lines: true } })) || updated
      );
    };
  ...
  async validateBeforeDelete(
    ...
    const current = await this.repository
      .getRepository(manager)
      .findOne({ where: { id: data.id }, relations: { lines: true } });
  ...
  async actionAfterDelete(
    ...
    const products = new Set<string>(
      ((data as any).lines || [])
        .map((line: InternalExportLine) => line.productId)
